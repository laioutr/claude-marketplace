...
> Search for existing components matching these names: `{componentNames}`. Search paths: `node_modules/@laioutr-core/ui-kit/src/runtime/app/components/` (atomic primitives), `node_modules/@laioutr-core/ui/src/runtime/components/` (commerce organisms). For each match, return: component path, props interface, and import alias. If the design might also exist as a Section/Block in your own module's `src/runtime/app/section/` or `src/runtime/app/block/`, search there too (this is rare — most matches are upstream).
