...
Concrete patterns for wrapping reka-ui primitives into your own atom-style components. Only applies when you author a custom primitive in your module that wraps reka-ui directly — consuming `@laioutr-core/ui-kit` components (which already wrap reka-ui upstream) does not invoke this rule.

Complements [`ui-kit-reka-ui.md`](./ui-kit-reka-ui.md) (accessibility + styling basics) and [`reka-ui-no-type-leakage.md`](./reka-ui-no-type-leakage.md) (the type-surface constraint this rule's typed-Props export is bound by).
