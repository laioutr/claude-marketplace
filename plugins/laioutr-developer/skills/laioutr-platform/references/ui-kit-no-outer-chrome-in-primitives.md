# No Outer Chrome on Primitive Components

If you author a custom primitive in your module — or fork one from `https://github.com/laioutr/ui-source` — its **bounding box must equal its visible footprint**. Spacing inside the control's own border is anatomy and stays. Spacing around the control is context and belongs to whoever places it.

When consuming upstream `@laioutr-core/ui-kit` primitives, expect this rule to already hold: their bounding box equals their visible footprint. If you need outer spacing, wrap them yourself — don't override their internals (see [`no-wrapper-css-overrides.md`](./no-wrapper-css-overrides.md)).

Inverse of [`no-wrapper-css-overrides.md`](./no-wrapper-css-overrides.md): that rule forbids the override; this one forbids the chrome that forces it.

## The rule

A primitive component must not bake outer padding, margin, container width, or surrounding background — anything that extends its bounding box past the visible control — unless the component is _chrome by design_ (see Exceptions).
...
```vue
<!-- ❌ SwiperNavigation.vue (custom primitive) -->
<style>
.swiper-navigation {
  display: flex;
  gap: var(--spacing-s);
...
1. **Composition fails when primitives have opinions about placement.** Two contexts, two margins, one component — the only escape is overrides, which [`no-wrapper-css-overrides.md`](./no-wrapper-css-overrides.md) forbids.
...
- [`three-layer-architecture.md`](./three-layer-architecture.md) — primitives are visually authoritative in shape, not placement
- [`ui-ui-kit-first.md`](./ui-ui-kit-first.md) — organism-style components compose `Backdrop`; controls inside must not bring competing chrome
