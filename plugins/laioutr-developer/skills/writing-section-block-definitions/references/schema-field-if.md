...
- Inside a `cardStyle` object field, reading a root-level `showStarRating`
  checkbox: `['get', '/showStarRating']`.
...
- [`section-config-standard.md`](./section-config-standard.md) — Rules panel sits in Studio's standard layout; visibility conditions are part of the broader Rules concept.
- [`forbidden-field-names.md`](./forbidden-field-names.md) — Field naming caveats `if` doesn't change.
