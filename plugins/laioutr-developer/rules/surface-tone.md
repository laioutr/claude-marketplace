...
How a container's painted appearance (light / dark / bright) propagates to descendants so that text, icons, buttons, and other visual elements pick contrasting colors automatically. This rule covers any ui-kit-, ui-, or ui-app-layer component in your Nuxt module.

> **Status: draft / evolving.** Phase 1 (Backdrop API consolidation) and parts of Phase 2/3/4 of the surface-tone refactor are still in flight in the upstream platform. Sections marked _(target state)_ describe the intended end state and may not be fully enforced yet.
...
> **Target state, not yet enforced**: a deprecated `containerBackground` prop on Backdrop / MediaText / Container is being removed across Phase 1. After that lands, the only route for a custom user color is `:background=` directly. Sections that historically routed via `:container-background=` are being migrated.

`Backdrop` runs `colorToSurfaceTone(color)` on custom values to derive the tone automatically. This works for hex inputs today; non-hex (`var(...)`, `light-dark(...)`, `rgb(...)`) currently short-circuit to `'light'` — that classifier is being extended in a separate work track.
...
## Portal handling _(target state)_

> **Not yet implemented.** Tasks #12–#14 in the surface-tone refactor build the `<TeleportSurface>` primitive and apply it to overlay components. Until then, portaled content (Sheet, Popover, Tooltip, Dialog, DropdownMenu, ContextMenu, Lightbox, Select, InputAutocomplete, InputCombobox, HoverCard) inherits Channel A but loses Channel B at the portal boundary, producing mixed-channel rendering bugs (e.g. SwatchChip in a Popover triggered from a dark hero).

When `<TeleportSurface>` lands, every overlay component will wrap its `<Portal>` content in it. Default behavior: portal content is a **fresh** surface (defaults to `'light'`) — does NOT inherit from the trigger's tone. This makes the bug shape "no tone, fall back to light" (predictable) rather than "wrong tone" (silent).

Authors who want trigger-tone inheritance opt in via composition:
...
Overlay components themselves do **not** add a `surfaceTone?` prop. The override pattern is composition.

## Hard rules (forbidden patterns)

1. **No literal tone strings inside ui-app-layer components.** A `tone="light|dark|bright"` literal in a Section or Block template defeats the CMS author's choice. If a section or block needs a fixed tone, expose it as a schema field. _(Not yet lint-enforced; planned.)_
...
> **Narrowing verdict (2026-05-08).** Some atoms narrow their `surfaceTone` prop via `Exclude<SurfaceTone, 'bright'>`. The current narrowings (`LanguageSwitcher`, `LanguageSwitcherDisplay`, `NavigationMenuTextItem`, `NavigationMenuTextItemWrapper`, `DarkModeSwitch`, `MegaMenu`, `MegaMenuContent`, `PageNotFound`) all live in **chrome / navigation / error-page** contexts where bright surfaces are not a designed placement. The narrowing reflects design intent and stays. **New components default to the full `SurfaceTone` union**; narrow only with a one-line comment naming the design constraint, and widen + add `bright` CSS the first time a component is genuinely placed on a bright surface (Storybook story + visual sign-off).
...
- `no-wrapper-css-overrides.md` — the rule that paired tokens replace; wrappers must not hardcode colors targeting child BEM
- `public-css-api.md` — broader public CSS contract; `data-surface-tone` and `.on-{tone}` are the documented selectors of OnSurface
