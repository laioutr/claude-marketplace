...
`--fix` cannot solve this — reordering would require moving imports **across script blocks**, which the autofixer will not do. The husky pre-commit hook then rejects the commit.
