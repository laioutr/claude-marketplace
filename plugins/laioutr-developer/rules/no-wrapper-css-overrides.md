...
This is one face of the broader [Public CSS API](public-css-api.md) — the prop-not-selector mechanism for cross-component styling. See [ADR 0013](../../docs/adr/records/0013-public-css-styling-api.md) for the full design. Once the `:ui` slot-class prop ships in vNext, it becomes the typed escape hatch for cases that today reach for `:deep()`.
...
3. **Cross-package coupling.** When a `ui` component overrides `ui-kit` class names, a change in ui-kit requires coordinated edits in ui without any type-level signal. The three-layer architecture (`ui-kit → ui → ui-app`) only works if each layer's surface is its props, not its DOM.
...
- [ADR 0013 — Public CSS Styling API](../../docs/adr/records/0013-public-css-styling-api.md) — the full design and rationale
